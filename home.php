	...
	<div class="container">
		<div class="row">
			<div class="col-sm-3">
				...
				<p>1. <a href="exam-schedule">Examination Informations</a></p>
				<p>2. <a href="eligibility-criteria">Eligibility Criteria for Examination</a></p>
				<p>3. <a href="student-course-login">Download Admit Card</a></p>
				<p>4. <a href="student-course-login">Download Results</a></p>
...
<dl>
			...
			if(isset($_POST['submit'])) 
			{
				...
				if ($num==1)
				{
					if($code==$_POST['code'])
					{
						...
						{
							setcookie('error',"<p class='alert alert-warning text-center'>First, Activate Your Account From Your
									Email Link.</p>",time()+3);
					...
					{
						setcookie('error',"<p class='text-center alert alert-danger'>Capcha is Mismatched.</p>",time()+3);
						header('Location:home');
					}
				} // first if closes
				else
				{
					setcookie('error',"<p class='text-center alert alert-danger'>Login Id or Password May be Incorrect.</p>",time()+3);
					header('Location:home');
				}
			}
		?>
	...
	<form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" onsubmit="return StudentValidation()">
		<fieldset>
				...
				<br>
				<div class="input-group">
					<p class="text-center"><a href="forget-password-one" name="forgetpass1">Forget Password???.</a></p>
					<p class="text-center">New Students Register here. <a href="sign-up" class="reg">Register Here.</a>
					</p>
				</div>
		</fieldset>
	</form>
