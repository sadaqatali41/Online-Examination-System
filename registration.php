			...
			if(isset($_POST['submit']))
			{
				...
				if($code==FilterData($_POST['code']))
				{
					...
					if(mysqli_affected_rows($con)>0)
					{
						...
						{
							setcookie('error',"<p class='alert alert-success text-center'><strong>Your Account is Created
									Successfully, Check Your Email For Login ID and Password and Activate Your Account<br>THANKS.</strong></p>",time()+3);
							header('Location:sign-up');
						}	
						else
						{
							setcookie('error',"<p class='alert alert-danger text-center'>
									<strong>Email Send Failed, Contact to Admin at sadaqatali890@gmail.com</strong></p>",time()+3);
							header('Location:sign-up');
						}
					}
					else
					{
						setcookie('error',"<p class='alert alert-warning text-center'><strong>This Email Id ".$email." or Mobile Number ".$phone." can't be same.</strong></p>",time()+3);
						header('Location:sign-up');
					}
					mysqli_close($con);
				}
...
<legend align="center"><h2>Sign-Up Form</h2></legend>
			...
			<div class="text-center">
				<h4 class="text-success">Already Registered Students <a href="student-course-login">Login Here.</a></h4>
			</div>
</fieldset>
</form>
		</div>
